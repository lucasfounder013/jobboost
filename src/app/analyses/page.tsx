...
import TraiterSauvegardePending from "@/components/modeles-cv/TraiterSauvegardePending";
...
function DashboardInner() {
  ...
  // Vue active — initialisée depuis ?vue= si présent, resynchronisée quand l'URL change (navigation sidebar).
  const vuesValides = ["historique", "nouvelle-analyse", "preparer-entretien", "trouver-rh"] as const;
  ...
  useEffect(() => {
    const p = searchParams.get("vue") as Vue | null;
    if (p && vuesValides.includes(p) && p !== vue) setVue(p);
  ...
  // ── Réinitialiser les états liés à la vue lors d'une transition (navigation sidebar)
  const vuePrecedenteRef = useRef<Vue>(vue);
  useEffect(() => {
    if (vuePrecedenteRef.current === vue) return;
    vuePrecedenteRef.current = vue;
    if (vue === "nouvelle-analyse") {
      setResultat(null);
      setCvAdapte(null);
      setErreur("");
      setCv("");
      ...
      setModeCV("upload");
      setAnalyseId(null);
    } else if (vue === "preparer-entretien") {
  ...
  async function ajouterAuxCandidatures(analyse: AnalyseSauvegardee) {
    if (analyse.candidature_id) return; // Déjà suivie
    const res = await fetch("/api/candidatures", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        poste: analyse.nom_offre,
        entreprise: "",
        statut: "postulee",
    ...
    setAnalyses((prev) => prev.map((a) => (a.id === analyse.id ? { ...a, candidature_id: json.id } : a)));
  }
      ...
      <AppSidebar
        ...
        quotas={
          chargementHistorique
            ? null
            : {
                scans: scansRestants ?? 0,
                credits: creditsRestants ?? 0,
                lmCredits: lmCreditsRestants ?? 0,
      ...
      <main className="md:ml-64 flex-1 overflow-y-auto">
                    ...
                    >
                      ...
                      <div className="flex flex-col gap-0.5 min-w-0">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {analyse.candidature_id ? (
                            <Link
                              href="/dashboard"
                              onClick={(e) => e.stopPropagation()}
                              className="inline-flex items-center gap-1 text-[11px] font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 rounded-full px-1.5 py-0.5 transition-colors"
                              title="Voir dans le suivi"
                            >
                            ...
                            <button
                              onClick={(e) => { e.stopPropagation(); ajouterAuxCandidatures(analyse); }}
                              className="inline-flex items-center gap-1 text-[11px] font-semibold text-indigo-600 hover:text-white bg-indigo-50 hover:bg-indigo-500 rounded-full px-1.5 py-0.5 transition-colors"
                              title="Ajouter au suivi de candidatures"
                            >
                              <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {vue === "trouver-rh" && (
          <div className="max-w-3xl mx-auto px-8 py-10">
            <div className="mb-8 flex items-start justify-between">
              <div>
                ...
                {estAbonne && rhCreditsRestants !== null && (
                  <p className="text-sm mt-1 font-medium text-indigo-600">
            ...
            {/* Compteur crédits — abonnés uniquement */}
            {estAbonne && rhCreditsRestants !== null && (
              <div className="mb-5 flex items-center gap-3 px-5 py-3.5 rounded-2xl bg-gradient-to-r from-indigo-50 to-violet-50 ring-1 ring-indigo-100">
                <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0 bg-indigo-100">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold uppercase tracking-wide mb-0.5 text-indigo-400">Crédits email</p>
            ...
            <div className="bg-white rounded-2xl ring-1 ring-gray-200 shadow-sm p-6 mb-6">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <button
                    ...
                    disabled={chargementDirect || !inputPrenomDirect || !inputNomDirect || !inputEntrepriseDirect || (estAbonne && rhCreditsRestants !== null && rhCreditsRestants === 0)}
